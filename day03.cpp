...
public:
    ...
    {
        return Employee::compareEmployees(a, b);
    }
};
class EmpList
{
...
int main()
{
    ...
    if (readInput == 1)
    {
        ...
        if (directory != nullptr)
        {
            ...
            cin >> key;
            if (key >= 0 && key < map.size() && map.find(key) != map.end())
            {
                fileName = map[key];
                fstream newfile;
    ...
    while (true)
    {
        ...
        cout << "7.save:" << endl;
        cout << "8.sort the Employees List :" << endl;
        cout << "9.exit :" << endl;
